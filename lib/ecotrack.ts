...
const ECOTRACK_API_URL = process.env.NEXT_PUBLIC_ECOTRACK_API_URL || 'https://swift.ecotrack.dz/api/v1';
...
class EcoTrackClient {
  private async request<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
    ...
    if (!response.ok) {
      ...
      const error = new Error(`EcoTrack API error: ${response.status} ${response.statusText}`);
  ...
  async validateToken(): Promise<EcoTrackTokenResponse> {
    return this.request<EcoTrackTokenResponse>(`/validate/token?api_token=${ECOTRACK_API_TOKEN}`);
  }

  /**
   * Get delivery fees for all active wilayas.
   * @returns List of fees per wilaya for different services
   */
  ...
  /**
   * Get list of products with stock information.
  ...
  /**
   * Request a return for a parcel.
   * @param tracking Tracking number
   */
  async askForReturn(tracking: string): Promise<EcoTrackReturnResponse> {
    return this.request<EcoTrackReturnResponse>(`/ask/for/order/return?tracking=${tracking}`, {
      method: 'POST',
    });
  }

  /**
   * Get tracking history and operations for an order.
   * @param tracking Tracking number
   */
  async getTrackingInfo(tracking: string): Promise<EcoTrackTrackingInfo[]> {
    return this.request<EcoTrackTrackingInfo[]>(`/get/tracking/info?tracking=${tracking}`);
  }

  /**
   * Filter orders by status and trackings.
   * @param trackings List of tracking numbers
   * @param statuses List of statuses to filter by
   */
  async getOrdersStatus(trackings: string[], statuses: (EcoTrackOrderStatusType | string)[]): Promise<EcoTrackOrdersStatusResponse> {
  ...
  /**
   * Get a paginated list of orders with optional date filtering.
   * @param page Page number
   * @param startDate Filter by creation date (Y-m-d)
   * @param endDate Filter by creation date (Y-m-d)
   */
  async getOrders(page: number = 1, startDate?: string, endDate?: string): Promise<EcoTrackOrdersResponse> {
  ...
  /**
   * Get list of active wilayas.
   * @returns List of deliverable wilayas
   */
  ...
  /**
   * Get list of deliverable communes.
   * @param wilayaId Optional filter by wilaya_id (1-58)
   * @returns List of deliverable communes
   */
  ...
  // Liste des commandes avec status
}
